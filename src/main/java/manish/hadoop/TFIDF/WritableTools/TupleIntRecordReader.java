...
public class TupleIntRecordReader extends RecordReader<TupleWritable, IntWritable> {
    private long start;
    private long end;
    private long pos;
    ...
    public boolean nextKeyValue() throws IOException, InterruptedException {
        ...
        try {
            String temp = outputvalues.remove(0);
            temp = temp.replaceAll("\\s+", "").trim();
            value.set(Integer.valueOf(temp));
        } catch (NumberFormatException nfe) {
            System.out.println("Unable to convert into int");
        }
